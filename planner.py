...
def parse_para_dict(output_text: str) -> dict:
    ...
    if not split_ratio_match:
        raise ValueError("Failed to parse 'Final split ratio' from planner output.")

    if not prompt_match:
        raise ValueError("Failed to parse 'Regional Prompt' from planner output.")

    para_dict = {
        "Final split ratio": split_ratio_match.group(1).strip(),
        "Regional Prompt": prompt_match.group(1).strip(),
    }
